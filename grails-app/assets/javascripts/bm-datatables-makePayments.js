...
$(document).ready(function() {
    ...
    function updateAfterFill(){
        $("#valueExpense.value-detailedMakePayments").val(sumDebtValue().toFixed(2));

        $("#users-detailedMakePayments div.col-md-6").first().addClass("col-xs-6");
        $("#users-detailedMakePayments div.col-md-6").last().addClass("col-xs-6 col-md-6");

        $('#users-detailedMakePayments_wrapper input').addClass("input-medium"); // modify table search input
        $("#users-detailedMakePayments_wrapper select").addClass("select2-wrapper span12"); // modify table per page dropdown
        ...
        if(!flag){
            $("#ToolTables_users-detailedMakePayments_0").hide();
            flag = true;
        }

        $("#valueExpense.value-detailedMakePayments").trigger("keyup");
        alreadyKeyUp = false;
    }

    $(".div-btn-widget-payments button.whoIOwe").click(function(){
        ...
        var tableElement = $('#users-detailedMakePayments');
        ...
        tableElement.dataTable( {
            ...
            fnPreDrawCallback: function () {
                ...
                $('#users-detailedMakePayments').find("tr.odd,tr.even").unbind("click");
                firstTime = true;
            },
    ...
    function addEventChangeInput(){
        $("#valueExpense.value-detailedMakePayments").keyup(function(){
            alreadyKeyUp = true;
            ...
            listValues.forEach(function(entry){
                ...
                updateRows(listCkecked[position], position, $(this), $("#users-detailedMakePayments tbody tr:nth(" + position + ")"));
                position++;
            });
            position = 0;
        });
    ...
    function addEventClick(nRow, aData, iDisplayIndex){
        $(nRow).click(function(e){
            ...
            if($("#valueExpense.value-detailedMakePayments").val() == 0){
                var pos = 0, sum = 0;
                ...
                $("#valueExpense.value-detailedMakePayments").val(sum.toFixed(2));
                alreadyKeyUp = false;
            }
            if(listCkecked[iDisplayIndex] == true){
                ...
                if(alreadyKeyUp && sumDebtValue() >= $("#valueExpense.value-detailedMakePayments").val()) return;
                listCkecked[iDisplayIndex] = true;
            ...
            var context = $("#valueExpense.value-detailedMakePayments");
    ...
    function updateChecks(index){
        var ctx = $("#users-detailedMakePayments tbody tr:nth(" + index + ")");
    ...
    $(".btn-submit-new-detailedMakePayments").click(function(){
